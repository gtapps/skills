...
- **Fallback ladder**: Artifact tool unavailable or publish fails → send the HTML via SendUserFile with `display: 'render'` → bare CLI, print the path. Local-HTML is also a *choosable* mode: publishing ships diff content to claude.ai, so if the operator asks for a local/private report, skip the Artifact tool deliberately.
